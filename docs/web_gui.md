...
- **提交限制**：只有全部 pair 预览成功且最新变更方案（ChangePlan）计算完成后，“应用 (Apply)”按钮才会启用。单个平台 warning 不阻断 Apply，但 pair 级预览失败会整批阻断并在队列中显示错误。
- **会话令牌**：会话令牌只保存在当前页面的 Pinia 内存状态中，所有后续请求和下载都通过 Bearer Header 鉴权；刷新页面会结束当前本地会话上下文。
- **重复上传**：每次选择或拖入的文件构成一个完整新批次，会替换会话中的上一批文件，不会追加。
